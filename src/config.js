...
export const CONFIG = {
  ...
  // же множителю (текущий шаг — ×1.78 от предыдущей версии).
  chukoScale: 2.4,
  khanScale: 2.8,
  chukoSize: {
  ...
  // --- Рассыпание ---
  scatter: {
    // Радиус/джиттер подняты: при исходных значениях все 16 объектов
    // падали плотной кучкой в центре поля — визуально мелко и с
    // перекрывающимися хитбоксами (несколько чүкө тапались одним касанием).
    radius: 1.3, // радиус зоны над центром поля
    heightMin: 0.9,
    heightMax: 1.25,
    heightJitterPerObject: 0.035,
    positionJitterXZ: 0.6,
    sideImpulseMax: 0.9,
  },
  ...
  // --- Валидатор раскладки ---
  layoutValidator: {
    minDistanceForFlick: 0.25, // минимальная дистанция между парой для комфортного щелчка
    maxStackHeightDelta: 0.05, // допустимая разница по Y при близких X/Z, иначе считается стопкой
    stackXZThreshold: 0.04,
